...
class Model(object):
    ...
    def action_after_GtoR(self):
        print('////////////////////////', self.state,
              '////////////////////////')
        self.scope.run_control_state = 'NO_EGG_AFTER_FLY'
    ...
    def __init__(self):
        self.states = [{
            ...
            'name': 'PREPARE',
            'volatile': VolatileClassPrepare
        }, {
            'name': 'RUN',
            'volatile': VolatileClassRun
        }, {
            'name': 'HATCH',
            'volatile': VolatileClassHatch
        }, {
            'name': 'GET',
            'volatile': VolatileClassGet
        }]

        self.transitions = [{
            'trigger': 'fromSTANBYtoPREPARE',
            'source': 'STANBY',
            'dest': 'PREPARE',
            'after': 'action_after'
        }, {
            'trigger': 'fromPREPAREtoRUN',
            'source': 'PREPARE',
            'dest': 'RUN',
            'after': 'action_after_PtoR'
        }, {
            'trigger': 'fromRUNtoHATCH',
            'source': 'RUN',
            'dest': 'HATCH',
            'after': 'action_after'
        }, {
            'trigger': 'fromHATCHtoRUN',
            'source': 'HATCH',
            'dest': 'RUN',
            'after': 'action_after_HtoR'
        }, {
            'trigger': 'fromRUNtoGET',
            'source': 'RUN',
            'dest': 'GET',
            'after': 'action_after'
        }, {
            'trigger': 'fromGETtoRUN',
            'source': 'GET',
            'dest': 'RUN',
            'after': 'action_after_GtoR'
        }]
