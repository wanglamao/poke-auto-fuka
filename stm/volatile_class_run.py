...
import time
import show.drawing as drawing
...
class VolatileClassRun(object):
    ...
    def detect_breeder(self, frame):
        ...
        frame_resize = cv2.resize(
            frame, (VolatileClassRun.width, VolatileClassRun.height))
        if self._detect_frame_count == 0:
            if VolatileClassRun.ALGOLISM == 'AKAZE':
                self._kp2, self._des2 = self._akaze.detectAndCompute(
                    frame_resize, None)
            elif VolatileClassRun.ALGOLISM == 'SURF':
                self._kp2, self._des2 = self._surf.detectAndCompute(
                    frame_resize, None)
            elif VolatileClassRun.ALGOLISM == 'USURF':
                self._kp2, self._des2 = self._surf.detectAndCompute(
                    frame_resize, None)
                ...
                self._kp2, self._des2 = self._brief.compute(
                    frame_resize, self._kp2)
        ...
        if len(self._good) > VolatileClassRun.MIN_MATCH_COUNT:
            if self._des2 is not None:
                src_pts = np.float32([
                    self._kp1[m.queryIdx].pt for m in self._good_without_list
                ]).reshape(-1, 1, 2)
                dst_pts = np.float32([
                    self._kp2[m.trainIdx].pt for m in self._good_without_list
                ]).reshape(-1, 1, 2)
                ...
                if self.center_bottom[0][0] >= 500 and self.center_bottom[0][
                        0] <= 800:
                    self.center_flg = 1
                else:
                    self.center_flg = 0
    ...
    def zncc(self, frame):
        ...
        match1a = cv2.matchTemplate(gray1a, self.gray_cross_arm_breeder1,
                                    cv2.TM_CCOEFF_NORMED)
        match2a = cv2.matchTemplate(gray2a, self.gray_cross_arm_breeder2,
                                    cv2.TM_CCOEFF_NORMED)
        match3a = cv2.matchTemplate(gray3a, self.gray_cross_arm_breeder3,
                                    cv2.TM_CCOEFF_NORMED)
        ...
        match1 = cv2.matchTemplate(gray1, self.gray_breeder1,
                                   cv2.TM_CCOEFF_NORMED)
        match2 = cv2.matchTemplate(gray2, self.gray_breeder2,
                                   cv2.TM_CCOEFF_NORMED)
        match3 = cv2.matchTemplate(gray3, self.gray_breeder3,
                                   cv2.TM_CCOEFF_NORMED)
        ...
        if max_value >= max_value_a:
            if pt[1] <= 11:
                ...
                draw_w = self.w1
                pt_1 = pt[1] + 372
            cv2.rectangle(self.action_frame, (pt[0], pt_1),
                          (pt[0] + draw_w, pt_1 + draw_h), (0, 200, 0), 3)
        else:
            pt = pt_a
            if pt[1] <= 11:
                draw_h = self.ah3
                ...
                pt_1 = pt[1] + 372
            cv2.rectangle(self.action_frame, (pt[0], pt_1),
                          (pt[0] + draw_w, pt_1 + draw_h), (0, 0, 200), 3)
    ...
    def f_zncc(self, frame):
        ...
        match_f = cv2.matchTemplate(gray_f, self.gray_f_breeder,
                                    cv2.TM_CCOEFF_NORMED)
        match_f_a = cv2.matchTemplate(gray_f, self.gray_f_a_breeder,
                                      cv2.TM_CCOEFF_NORMED)
        _, max_value_f, _, _ = cv2.minMaxLoc(match_f)
        _, max_value_f_a, _, _ = cv2.minMaxLoc(match_f_a)
        if max_value_f > max_value_f_a:
            print('no egg after fly')
            self.check_egg_frames += 1
            if self.check_egg_frames >= 5:
                self.check_egg_frames = 0
                self.run_control_state = 'NO_EGG_AFTER_FLY'
        else:
            print('a')
            self.check_egg_frames = 0
            self.run_control_state = 'GET_EGG_AFTER_FLY'

    def run_control(self, frame):
        # print(self.run_control_state)
        if self.run_control_state == 'AP':
            self.from_door_to_center_road_then_run_right()
            ...
            self.turn_r()
        elif self.run_control_state == 'TURN L':
            self.turn_l()
        elif self.run_control_state == 'SAVE':
            self.save_game()
            ...
            self.f_zncc(frame)
        elif self.run_control_state == 'GET_EGG_AFTER_FLY':
            self.run_get(frame)
        elif self.run_control_state == 'NO_EGG_AFTER_FLY':
            self.turn_r()
    ...
    def save_game(self):
        now = time.time()
        # print(now - self.last_save_time)
        print(now - drawing.last_save_time)
        # if now - self.last_save_time >= 600.0:
        if now - drawing.last_save_time >= 600.0:
            if self._control_frame_count == 0:
                self.send_command = 'Button X'
                self._control_frame_count += 1
            elif self._control_frame_count == 3:
                self.send_command = 'STOP'
                self._control_frame_count += 1
            elif self._control_frame_count == 15 + 30:
                self.send_command = 'Button R'
                self._control_frame_count += 1
            elif self._control_frame_count == 17 + 10 + 30:
                self.send_command = 'Button A'
                self._control_frame_count += 1
            elif self._control_frame_count == 19 + 10 + 30 + 30:
                self.send_command = 'STOP'
                self._control_frame_count += 1
            elif self._control_frame_count == 30 + 10 + 30 + 60:
                self.send_command = 'Button A'
                self._control_frame_count += 1
            elif self._control_frame_count == 120 + 30 + 60 + 60:
                self.send_command = 'Button B'
                self._control_frame_count += 1
            elif self._control_frame_count == 125 + 30 + 60 + 60:
                self.send_command = 'STOP'
                self._control_frame_count += 1
            elif self._control_frame_count == 130 + 30 + 60 + 60 + 60:
                self.send_command = 'Button B'
                self._control_frame_count += 1
            elif self._control_frame_count == 131 + 30 + 60 + 60 + 60 + 60:
                self.send_command = 'STOP'
                self._control_frame_count = 0
                print("game saved")
                drawing.last_save_time = now
    ...
    def from_door_to_center_road_then_run_right(self):
        if self._control_frame_count == 0:
    ...
    def after_getting_egg_go_to_runway(self):
        if self._control_frame_count == 10:
            self.send_command = 'Button START'
            self._control_frame_count += 1
        # elif self._control_frame_count == 1 + 10:
        #     self.send_command = 'LY MAX'
        #     self._control_frame_count += 1
        elif self._control_frame_count == 10 + 10:
            self.send_command = 'I LX MAX'
            self._control_frame_count += 1
        elif self._control_frame_count == 10 + 12:
            self.send_command = 'STOP'
            self._control_frame_count += 1
        elif self._control_frame_count == 11 + 10 + 5:
            self.send_command = 'Button START'
            self._control_frame_count += 1
        elif self._control_frame_count == 11 + 10 + 15:
            self._control_frame_count = 0
            self.run_control_state = 'RUN R'
        else:
            self.send_command = 'None'
    ...
    def run_r(self):
        if self._control_frame_count == 0:
            ...
            self.send_command = 'LX MAX'
            self._control_frame_count += 1
        elif self._control_frame_count == 205 + 30:
            self.send_command = 'STOP'
            self._control_frame_count += 1
        elif self._control_frame_count == 220 + 30:
            self.send_command = 'STOP'
            self._control_frame_count = 0
            self.run_control_state = 'TURN L'
        else:
            self.send_command = 'None'
            self._control_frame_count += 1
    ...
    def turn_l(self):
        if self._control_frame_count == 20 - 20:
            self.send_command = 'I LX MIN'
            self._control_frame_count += 1
        elif self._control_frame_count == 21 - 20:
            self.send_command = 'STOP'
            self._control_frame_count += 1
        elif self._control_frame_count == 30 - 20:
            self.send_command = 'None'
            self._control_frame_count = 0
            self.run_control_state = 'RUN L'
        else:
    ...
    def fly(self, frame):
        # print('fly', self._control_frame_count)
        if self._control_frame_count == 0:
            #self.send_command = 'Button START'
            self._control_frame_count += 1
    ...
    def menu_sel_det(self, frame):
        ...
        menu_poke_tmp = np.int8(self.menu_icon_pokemon)
        dif_menu_poke = np.amax(abs(menu_poke_tmp - cut_menu_frame_poke))
        print("dif_menu_pokemon " + str(dif_menu_poke))
        if dif_menu_poke <= 60:
