...
class DrawingClass(object):
    ...
    def drawing(self, frame, fps, num_egg, htc_egg, state, time_delta,
                shiny_number):
        cv2.putText(frame,
                    'FPS: {:.2f}'.format(fps), (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 0),
                    thickness=2)

        cv2.putText(frame,
                    'Possessed EGG: {}'.format(num_egg), (10, 100),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 255),
                    thickness=2)
        cv2.putText(frame,
                    'Hatched EGG: {}'.format(htc_egg), (10, 130),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 255),
                    thickness=2)
        cv2.putText(frame,
                    'Running Time: {}'.format(time_delta), (10, 160),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 255),
                    thickness=2)

        cv2.putText(frame,
                    'State: {}'.format(state), (250, 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 255),
                    thickness=2)
        cv2.putText(frame,
                    'Shiny: {}'.format(shiny_number), (250, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0, (0, 255, 255),
                    thickness=2)
        return frame
